...
def compute_bonus_reward(original_reward, prev_distance, current_distance, bonus_factor=1.0):
    ...
    Only positive improvements (i.e. moving closer) yield a bonus.
    """
...
def sarsa_pacman(num_episodes=50, alpha=0.1, gamma=0.95, epsilon=1.0,
                 ...
                 bonus_factor=1.0):
    """
    SARSA implementation for MsPacman that uses discretized Pacman's (x,y) coordinates
    as state and adds a bonus if Pacman moves closer to a pellet.
    """
    n_actions = env.action_space.n  # Typically 9 actions.
    Q = {}  # Q-table: keys are (state, action) tuples.
    rewards_per_episode = []
...
def record_video(Q, num_bins_x, num_bins_y, video_folder="videos", video_prefix="sarsa_video"):
    """
    Records a rollout video using the learned Q-table.
    ...
    while not done:
        if state_disc not in Q:
            # If no Q value, take a random restricted action.
...
if __name__ == "__main__":
    ...
    min_epsilon = 0.1
    decay_rate = 0.995
    num_bins_x = 16   # Adjust based on the 160-width of MsPacman (e.g., 16 bins => 10 pixels per bin)
    num_bins_y = 21   # Adjust based on the 250-height of MsPacman (e.g., 21 bins => ~12 pixels per bin)
    bonus_factor = 1.0

    hyperparams = [(0.1, 0.95), (0.1, 0.90), (0.1, 0.85), (0.1, 0.80)]
    reward_curves = {}
